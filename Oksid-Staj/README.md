#  Oksid Uzun Dönem Staj Gelişim Günlüğü: Yapay Zeka ve Otomasyon Çözümleri

Bu repo, uzun dönem stajım boyunca edindiğim teknik bilgi, geliştirdiğim iş akışları (workflows), karşılaştığım zorluklar ve çözüm stratejilerimden oluşan kapsamlı bir **gelişim günlüğüdür**.

Amacım, bu süreçte öğrendiklerimi kalıcı bir **bilgi kaynağı** haline getirmek, teknik yetkinliklerimi belgelemek ve otomasyon/yapay zeka alanındaki pratik deneyimimi şeffaf bir şekilde paylaşmaktır.

-----

##  Repo Yapısı

Bu Repo, staj süresince odaklanılan konuları ve edinilen bilgileri kronolojik olarak takip etmenizi sağlayacak şekilde haftalık klasörlere ayrılmıştır.

```
.
├── 1.-hafta/
│   └── staj-gunlugu-1-hafta.md  // İlk haftanın detaylı teknik raporu
├── 2.-hafta/
│   └── ...
├── 3.-hafta/
│   └── ...
└── README.md
```

###  Haftalık Klasörlerde Neler Bulunur?

Her haftalık klasör (Örn: `1.-hafta/`), ilgili haftanın teknik odağını, geliştirilen iş akışlarını, karşılaşılan kritik problemleri ve öğrenilen yeni teknolojileri içeren detaylı bir `.md` raporu barındırır.

-----



##  İletişim

Bu projeler, notlar ve öğrenilenler hakkında sorularınız varsa veya deneyimlerimi daha detaylı incelemek isterseniz benimle iletişime geçebilirsiniz.
